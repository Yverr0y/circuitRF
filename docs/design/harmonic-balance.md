...
**What that costs, measured.** On the Hero-5 GaN PA (`testdata/Hero5/hero5_6tone.cnl`), single drive point, per POINT (Release, Apple M4, single-threaded except where the Jacobian's product fans out):

| shape | M | unknowns | per point |
|---|---|---|---|
| 6 tones, order 2 (as the fixture ships) | 43 | 172 | **7 ms** |
| 6 tones, order 3 | 189 | 756 | **181 ms** |
| 3 tones, order 3 | 32 | 128 | **5 ms** |

Both dense-Jacobian, no iterative solver. **These supersede the 0.4 s / 4.3 s figures this section carried until 2026-08-30**, which were end-to-end through the CLI (so they included process start, parse and elaboration) and, more to the point, predate HB-P1: the dense solve was a Gauss-Jordan sweep, the Jacobian's triple product a scalar loop, and the APFT was rebuilt once per sweep point. The order-3 point alone went from 903 ms to 181 ms of engine time on those three changes (`src/Engine/RESOLVED.md` §HB-P1). The first point of a session additionally pays the transform construction once — 92.6 ms at order 3, 2.4 ms at order 2 — and then every later point in the sweep reuses it (§6.5).

**Two tones run on this path too, by default since 2026-08-30.** `AnalysisSettings.HbTwoToneOnLattice` (default **true**) routes a two-tone analysis through the lattice instead of the rectangular FFT of §6.1–§6.3; clearing it routes them back, and §6.1–§6.3 stay in the tree as the independent second implementation the lattice is gated against. It is the same retained diamond either way and `MixingLattice` at `T = 2` reproduces `MixingGrid`'s enumeration element for element, so cube shapes, `mixIndex` labels, `TwoToneMeasurements` and the data display are unaffected; only the numbers move, and only in their last digits. Measured on `hero5.cnl` over the golden drive range: **21.3 ms/point → 6.1 ms/point (3.5×)**, because the FFT grid evaluates the device on 1,024 time samples to solve 62 complex unknowns where the lattice needs ~250. The disagreement between the two grows monotonically with mixing order — 1.5e-16 at DC, 1.8e-8 at the carriers, 4.8e-6 at IM3, 8.9e-2 at the order-5 rim — which is truncation, not error, and is why the rim of the retained set should not be read at face value on either path. The committed Hero-5 goldens were produced on the FFT path and still pass from the lattice, so they are now a cross-path check; see `src/Engine/RESOLVED.md` §HB-P1 for the measured margin.
...
`J` is `2·N_nl·(K+1)` square (single-tone). **`N_nl`, the number of nonlinear-facing nodes, is far smaller than the full node count** — a single-FET PA has a handful, Hero 4's two FETs still only a small block. So the HB Newton system is solved **dense**, distinct from the sparse CSparse path the *linear* MNA uses for the full netlist. Per iteration: factor `J`, solve `J · ΔV = −F`, update `V ← V + λ·ΔV` (with optional damping `λ ≤ 1`, §11). Block-structured or iterative HB solves (exploiting the harmonic/node block pattern) are a noted **future optimization** if the `< 3 s`/point NFR (PRD §14) is missed at Hero-4 scale; v1 is dense. **Multi-tone does not change that**, contrary to the pairing §16 item 7 originally assumed: at `T` tones the system is `2·N_nl·M` for retained-product count `M`, and §6.6 caps `M` at 600 precisely so the dense solve stays adequate.

**The factorisation is `HbNewton.SolveLu` — an in-place right-looking LU with partial pivoting on the `double[]` the Jacobian already lives in, with the rank-1 trailing update vectorised.** Every Newton path (single-tone, two-tone, T-tone) goes through the one entry point `HbNewton.SolveGaussian`, which keeps its `null`-on-singular contract and dispatches below `HbNewton.SolveCrossover` (measured: 8) to the original augmented Gauss-Jordan sweep, which is marginally faster at that size. **It is deliberately not NumFlat's `Mat<double>.Lu()`** — earlier revisions of this section said it was, describing code that did not exist. NumFlat was then measured against the in-house kernel on the matrices this engine actually produces and is slower everywhere that matters (39.5 ms against 29.4 ms at n = 756, erratic at power-of-two sizes, slower than Gauss-Jordan below n ≈ 40, and it needs a copy into and out of `Mat<double>`). It remains the independent oracle `HbDenseSolveTests` gates the shipped kernel against.
...
> **Setting: `DriveStepping`** `{ IfNecessary, Always, Never }`, default `IfNecessary` — the tri-state knob governing HB's RF-**drive** continuation, the analog of nonlinear DC's `DcBiasStepping` (which ramps the bias *supplies*). The two are deliberately **separate settings** because they ramp different continuation parameters: `DcBiasStepping` walks the **DC supplies** up to bias the device (Phase 3); `DriveStepping` walks the **RF input drive power** up into compression (this section). `IfNecessary` means: try the cheap start first — a **warm-start from the previous sweep point** (the common case across a power/Γ sweep), or the DC seed for the first point — and fall back to drive-ramping only if that fails. `Always` ramps from small-signal every point; `Never` attempts the warm/seed start only and reports non-convergence on failure.
>
> **AS BUILT (HB-P3 M2, 2026-08-30) — it was parsed and then ignored for two years.** `HbEngine.Resolve` read `DriveStepping=` into `HbAnalysisParams` and *nothing downstream read it back*; the knob this section reserved "for Phase-4 bring-up" was never wired. It now is, on all three tone counts, as `HbDriveRamp`:
>
> - **The rungs are a fixed 2 dB ladder over a 20 dB span**, offsets stated *relative* to the requested drive so the last rung is exactly 0 dB — the point reported is the point that was asked for, not an accumulated sum near it. 2 dB is loadpull's own measured step (it gave zero holes across its Γ grid).
> - **The drive moves through `IDriveScalable`** (`src/Core/Devices/`), a linear multiplier on a source's **tone excitation only, never its DC bias** — implemented by `ToneSourceModelBase` (`V_1Tone`/`V_nTone`/`I_1Tone`/`I_nTone`), `P1Tone` and `PnTone`. It is a *voltage* multiplier, `10^(d/20)`, which is the same arithmetic whether a source declares an amplitude or an available power, so one ramp moves every source in a circuit together by a common dB offset. Only `I_src` is re-extracted per rung — `Y_NN` is topology — so a rung costs one back-solve per harmonic on top of its Newton, and the engine restores the drive in a `finally`.
> - **A rung that does not converge is re-walked by `DriveLadder.ContinueThroughJump`**, the same subdivision the loadpull engine uses, written once. Its *other* half — the physical continuity criterion, stated in output power — is inert here: a generic `type=hb` testbench has no designated load node to compute one from, so the ramp rejects a sub-step on non-convergence only and passes `marginDb: 0` to say so.
> - **A ramp that never reaches the requested drive publishes nothing of its own**; the cold result stands, non-convergence and all (§11.1's rule that a wrong branch is never smuggled in as an answer).
>
> **In practice `IfNecessary` is now nearly free, because the line search below catches what used to land here.** It fires only where the DC seed is not on the answer's branch at all — a line search shortens a step, it cannot choose a different starting point.
...
- **Step backoff on failure.** If Newton hits the max-iteration cap without meeting tolerance (§12), **halve the continuation step** and retry from the last good solution; on repeated failure, report non-convergence at that point with the residual and the last step. (Damping `λ < 1` on the Newton update is the within-step companion knob — see §11.2, which is where that companion actually lives now.)

### 11.2 The Newton line search (as built, HB-P3 M1)

**The within-step half of continuation, and the one that turned out to matter most.** `Lambda` (§16 item 4's `λ`) was a *fixed* factor applied to every update, defaulting to 1 — i.e. the Newton was undamped by default. Measured on the shipped `hero2_convergence.cnl`, cold from a DC seed: 0 dBm converged in 3 iterations and 10 dBm in 4, but **16 and 20 dBm ran to `MaxIter = 100` and returned non-converged** (‖F‖ = 4.6e-1 and 9.6e-1). The Jacobian is exact there — `CompareJacobianNumerical` gates `BuildJ` at 1e-5 on that very fixture at 18 dBm — and warm from 1 dB below the same point converges in two or three iterations. **The failure was the step, not the direction:** a full step from the DC seed overshoots into the region where the SDD's `tanh`/`exp` terms saturate, and the iterate then wanders.

All three Newton loops (`HbNewton`, `HbNewton2D`, `HbNewtonNd`) now accept a step only if it reduces the residual, via `HbNewton.Backtrack`:

- **Armijo on ‖F‖:** accept `V + λ·ΔV` when `‖F(V+λ·ΔV)‖ ≤ (1 − c·λ)·‖F(V)‖` with `c = 1e-4`; otherwise halve λ and try again, up to **8 halvings** (λ down to 1/256, a constant with a comment, not a user setting). Exhausting them keeps the smallest step rather than standing still — standing still would rebuild the same Jacobian and re-reject the same step forever — and flags `Stalled` in the trace.
- **The criterion stays absolute, in amperes** (§12.2). The search compares ‖F‖ before and after a step; it introduces no relative measure, and §12.2's relative / normalize-to-drive alternatives stay unbuilt.
- **`Lambda` keeps its meaning as the user's override**: it is now the *first* λ tried rather than the only one. At the default 1 a converging solve takes a full step accepted on the first trial, so the goldens do not move.
- **One evaluation per accepted step plus one per rejected trial.** The accepted trial's evaluation *is* the next iteration's entry evaluation, so the loop never evaluates twice at the same `V`; `HbNewton.Evaluations` is the test-facing counter that pins it.
- `HbConvergenceTrace.IterRecord` carries `Lambda`, `Backtracks` and `Stalled` per iteration.

**Measured (Release, Apple M4).** The two failing points converge in 6 iterations each (4 and 6 backtracks). A cold 21-point Pin sweep of the same fixture, 0→30 dBm: **6.5 → 2.07 ms/point, 797 → 132 Newton iterations, 6 non-converged points → 0.** A *warm* sweep of the same fixture is unchanged at 85 iterations and pays ~5% (1.14 → 1.19 ms/point) for the per-iteration iterate copy.

**One premise this did not bear out.** A converging warm solve is *not* unconditionally byte-identical to the undamped loop: two of that sweep's 21 points (13.5 and 15.0 dBm, on the compression knee) take a full step whose ‖F‖ genuinely grows, and the search refuses it. Nineteen take zero backtracks. The frozen Hero-2/4/5 goldens are unmoved.
...
- Gated by **`AnalysisSettings.HbSweepWarmStart`** (default **on**). Set false to force a cold DC seed at every point (e.g. to study branch-dependence near a bifurcation).
- **Every tone count, since HB-P3 M3 (2026-08-30).** `RunTwoTone` and `RunMultiTone` used to build their guess from `NonlinearDcEngine.Run` + `InitialGuess2D` on *every* call and took no seed at all, so a swept two- or n-tone analysis cold-started every point while the single-tone one beside it had been chaining since this section was written. Both now take the same seed, of their own `[N, M]` lattice shape, and publish `InterfaceV`/`Converged`/`Trace` like the single-tone path; the sweep engine's chain is shape-agnostic because each engine path checks the shape it was handed. The innermost-axis-only rule and the reset-on-non-convergence rule are unchanged and apply at every tone count. Measured on `hero5.cnl` and `hero5_3tone.cnl`, 8 points from 8 to 22 dBm: **8 nonlinear-DC solves → 1**, Newton iterations 41 → 35 (two-tone) and 64 → 43 (three-tone), spectra agreeing to ~2e-8 relative.
...
- **Newton solve** is **dense** (`HbNewton.SolveLu`, an in-house vectorised partial-pivot LU — see §8 for why not NumFlat) — `N_nl` ≪ total nodes; block/iterative solves deferred, **including for multi-tone**: the §6.6 product ceiling keeps the T-tone system inside what dense handles.
...
4. ~~**Damping policy** (§8, §11)~~ — **SETTLED (HB-P3 M1, 2026-08-30): line-searched, engaged only after a failed full step — which is both of the last two options at once, and the reason the item read as a choice between them is that it was written before the failure was measured.** λ starts at the user's fixed `Lambda` (default 1, i.e. a full Newton step), and backtracking engages only when that step fails to reduce ‖F‖. So a solve that was converging before is untouched, and the cases that were not — the shipped Hero-2 fixture at 16 and 20 dBm, both burning `MaxIter = 100` — converge in 6 iterations. The full policy, its measurements and the one premise that did not survive contact are in **§11.2**; the continuation step-backoff it was to be tuned alongside is `DriveStepping`'s ramp (§11), now the fallback rather than the first move.
...
7. **True n-tone (≥ 3 tones)** (§6.4–§6.6) — **BUILT** (up to 6 tones). Two corrections to what this item predicted, both worth keeping visible:
   ...
   - It did **not** need to land with the iterative solver (item 5), contrary to the pairing asserted here and in the old §6.4. The product ceiling (§6.6) bounds the retained set at a size the dense solve handles: 6 tones at order 3 is a 181 ms point (§6.4). Item 5 stays deferred on its own merits.
