...
public class Hero3BPursuitTests(ITestOutputHelper output)
{
        ...
        BuildEngine(bool createLoadpullResult = false)
    {
        ...
        // The .cnl's `OutputGrid=` names loadpull_pursuit_output.gam INSIDE testdata/, so every test
        // that runs this engine rewrote committed data as a side effect — the engine writes the .gam
        // itself (LoadpullPursuitEngine, not the test). Point it at scratch so an ordinary run still
        // exercises that write path without touching the repo; a deliberate regeneration keeps the
        // real destination. See GoldenRegen.
        if (!GoldenRegen.Enabled && !string.IsNullOrEmpty(pp.OutputGridPath))
            pp = pp with
            {
                OutputGridPath = Path.Combine(Path.GetTempPath(), "circuitrf-hero3B-pursuit.gam")
            };
    ...
    public void GenerateHero3BGolden()
    {
        ...
        if (lpa.OutputGridPath is not null)
        {
            ...
            // The .gam is committed data like the CSVs; the same gate governs it.
            if (GoldenRegen.Enabled) GamWriter.WriteFile(lpa.OutputGridPath, gamResult);
