...
///   3. built-in provider     (the shipped .ccolor assets, embedded in this assembly)
///   4. <see cref="ColorTheme.BuiltIn"/> fallback (always succeeds)
/// </summary>
public static class ThemeResolver
{
    /// <summary>
    /// Step 3 of the chain. <b>Defaults to the framework-free reader below rather than to null</b>
    /// (R-rnd1-5, 2026-09-07).
    ///
    /// <para><b>Why that mattered.</b> This used to be installed only by <c>App.axaml.cs</c>, out of
    /// <c>avares://CircuitRF.Ui/Assets/Color/</c> through Avalonia's <c>AssetLoader</c>. With no app
    /// host it was simply never installed, so a theme name that resolves in the GUI fell through
    /// step 3 to <see cref="ColorTheme.BuiltIn"/> — a DIFFERENT PICTURE, reported as a success. The
    /// shipped <c>.ccolor</c> now lives in this assembly's own manifest resources and is read with
    /// <see cref="Assembly.GetManifestResourceStream(string)"/>, which needs no platform at all.</para>
    /// </summary>
    private static Func<string, ColorTheme?>? _builtInProvider = LoadEmbeddedTheme;

    /// <summary>
    /// The shipped themes, read out of THIS assembly. One file, <c>Default.ccolor</c> — see
    /// <see cref="BuiltInThemeNames"/>. Returns null for anything else, which is what lets the
    /// chain fall through to <see cref="ColorTheme.BuiltIn"/>.
    /// </summary>
    private static ColorTheme? LoadEmbeddedTheme(string name)
    {
        // The LogicalName the .csproj assigns: "CircuitRF.Render.Assets.Color.<name>.ccolor".
        using var stream = typeof(ThemeResolver).Assembly.GetManifestResourceStream(
            $"CircuitRF.Render.Assets.Color.{name}.ccolor");
        if (stream is null) return null;
        using var reader = new StreamReader(stream);
        try { return ColorThemeIo.Load(reader.ReadToEnd()); } catch { return null; }
    }
    ...
    /// <summary>
    /// Replaces the built-in provider. <b>Nothing in this repository needs to call it any more</b> —
    /// the default above already reads the shipped themes with no platform — but the seam stays so a
    /// host that ships its own bundled palettes can install one.
    /// </summary>
    public static void SetBuiltInProvider(Func<string, ColorTheme?> provider)
        => _builtInProvider = provider;

    /// <summary>
    /// Puts step 3 back to the embedded reader. <b>A test that replaces the provider must call this
    /// rather than installing a no-op</b>: since R-rnd1-5 the default is not "nothing", it is the
    /// shipped <c>.ccolor</c>, and leaving a no-op behind makes every LATER test in the process
    /// resolve every theme name to <see cref="ColorTheme.BuiltIn"/> — the exact silent fallback this
    /// brief removed, reintroduced by the test suite instead of by the code.
    /// </summary>
    public static void ResetBuiltInProvider() => _builtInProvider = LoadEmbeddedTheme;
    ...
    public static ColorTheme Resolve(string name, string? workspaceDirPath = null)
    {
        ...
        // 3. built-in provider (this assembly's own embedded .ccolor, unless replaced)
