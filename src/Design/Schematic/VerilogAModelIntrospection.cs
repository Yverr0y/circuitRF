...
public static class VerilogAModelIntrospection
{
    ...
    /// <summary>
    /// Where the terminal-label cache is kept, or <c>null</c> for no cache at all.
    ///
    /// <para><b>The one thing this file could not bring across the UI firewall.</b> The directory is
    /// this INSTALLATION's per-user state root — <c>AppDataRoot</c>, which stays in <c>src/Ui</c>
    /// because it is the single lever the docs factory redirects to make a capture reproducible, and
    /// moving it would drag the updater, the crash reporter and preferences down with it. Duplicating
    /// its arithmetic here would recreate the two-independent-callers problem it exists to have
    /// solved, so this asks for the directory instead of computing one
    /// (brief-automation-2 R-aut2-7 resolution 3; <c>LayoutTextOutline.TypefaceSource</c> is the
    /// precedent for the shape).</para>
    ///
    /// <para>Unset — which is what a headless <c>circuitrf</c> process gets — means every read is a
    /// cache miss and every write is dropped, which is exactly what an unreadable cache directory has
    /// always meant here: the labels are re-derived by the next describe. <c>src/Ui</c> installs the
    /// real directory from a module initializer, so all three of its entry points get it (R-aut2-9).</para>
    /// </summary>
    public static Func<string?> CacheDirectory { get; set; } = static () => null;

    private static string? StorePath =>
        CacheDirectory() is { } dir ? Path.Combine(dir, StoreFileName) : null;
