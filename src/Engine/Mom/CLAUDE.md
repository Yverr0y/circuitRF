...
- Edge grading reuses R-mom-8's cell-size FIELD and `GeometricSlopeFor`, but **kernel B marches it
  ...
  at 1.7. `EdgeFractionOfReference` = 0.03, `EdgeGrowthRatio` = 1.7 — **but the REALISED fraction is
  `max(3%, 1/(MinCellsAcross · MaxEdgeRefinement))`** (EFAN, 2026-09-15): 3% of the metal said nothing
  about the mesh that metal is meshed at, so coarsening the mesh LENGTHENED the fan. Inert at 4 across
  and above, so the default is bit-identical. `RESOLVED.md` §EFAN.
...
| `PlanarMeshSettings.MinCellsAcrossConductor` | 4 | sets transverse pitch; **does not respond to λ**. Since EFAN it caps the edge cell too (below), so it acts on an edge-meshed mesh rather than 13× less. |
...
| `PlanarMeshSettings.EdgeCells` | — | reference length = **the metal at each edge** (`PlanarEdgeReference.LocalConductorWidth`, default since 2026-09-09; was the narrowest conductor anywhere). May only coarsen — every per-edge `c₀` is floored at the global one — so the cell count is bounded above by the old rule's, structurally. **EFAN caps it as well: never finer than `MinCellsAcross × MaxEdgeRefinement` (10) under the pitch that control asks for** — measured against kernel A, flat above ~8.5 and WORSE above ~34 (the port cell sets `a₂₁`). `RESOLVED.md` §M4, §EFAN. |
