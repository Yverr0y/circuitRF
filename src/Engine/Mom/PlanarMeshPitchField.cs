...
/// <summary>
/// The per-axis bulk pitch the transmission-line mesh asks for, sampled and graded, plus everything
/// the report needs to say about how it was arrived at.
/// </summary>
/// <param name="Ok">False when the artwork gave no usable direction anywhere; the mesher then falls
/// back to the per-axis rule and the setting does nothing, with <paramref name="Note"/> saying why.</param>
...
/// <param name="WorstAspect">The largest cell aspect the field asks for, after the cap.</param>
/// <param name="Capped">Whether <see cref="PlanarMeshSettings.MaxCellAspect"/> actually bound.</param>
...
public static class PlanarMeshPitchField
{
    ...
    /// <param name="hWave">λ_g / CellsPerWavelength — the ALONG requirement, and the only one.</param>
        ...
        IReadOnlyList<PlanarPort>? ports = null)
    {
        ...
        if (!(w > 0) || !(h > 0) || !(hWave > 0) || double.IsInfinity(hWave) || minCellsAcross < 1
            || !(floorX > 0) || !(floorY > 0))
            return Declined("the transmission-line mesh needs a finite wavelength cell and a bounded " +
                            "piece of artwork; this problem has neither.");
        ...
        if (!anyMetal)
            return Declined("no field sample landed on metal, so there is no direction to follow.");
        ...
        static PlanarPitchField Declined(string why) =>
            new(false, "Transmission-line mesh NOT applied — " + why + " Used the per-axis rule.",
                0, 0, 0, 0, 1, false);
    }
