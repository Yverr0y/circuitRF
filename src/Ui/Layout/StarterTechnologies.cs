...
// modelled in the wrong place. MmicGaAs carries its MIM capacitor module inline (MIM-7 folded the
// former separate MmicGaAsMim back in — the capacitor dielectric is TIED to its plate, so a run
// with no plate artwork in it does not carry the film at all). Empty() (added for L0d's "New Technology…" picker) is deliberately
...
public static class StarterTechnologies
{
    ...
    public static Technology MmicGaAs()
    {
        ...
        // MIM-2's two module layers, MIM-7's to keep. Appended, never renumbered: a design drawn
        // before the module existed opens with every layer meaning exactly what it did.
        ...
        var tech = new Technology
        {
            ...
            [
                ...
                // The plate metal draws ABOVE both interconnect metals (ZOrder 10) because a
                // capacitor's top plate is a small patch on a usually-larger Metal1 bottom plate —
                // under it, it would be invisible in the one view where it matters. Its via takes
            ...
            Stackup = new Stackup
            {
                ...
                [
                    ...
                    new StackupLayer
                    {
                        // 2.55 µm, not 3: the plate metal (0.25) and its dielectric (0.2) come out
                        // of the air gap, so Metal2 still sits EXACTLY 3 µm above Metal1 — which is
                        // what makes an interconnect-only run on this technology bit-identical to
                        // the same run on the pre-MIM-2 stack (MIM-7's own gate).
                        Kind = StackupKind.Dielectric, Name = "Air",
                        ThicknessDbu = Um(2.55m), Epsr = 1.0, TanD = 0,
                    },
                    // ── The MIM capacitor module (MIM-2), tied to its plate (MIM-7) ──────────────
                    new StackupLayer
                    {
                    ...
                    new StackupLayer
                    {
                        // The top plate's connection up to the routing metal. Drawn as a REGION
                        // (MIM-1) rather than a point via — a plate connection is a patch nearly as
                        // large as the plate, not a barrel.
    ...
    // ── MIM-7 — there is no longer a second MMIC technology, and this is the note that says so ──
    //
    // MmicGaAsMim() existed from MIM-2 to MIM-7 because stating a capacitor dielectric between the
    // interconnect metals cost two measured things on EVERY run: an airbridge post between those
    // metals crossed a dielectric interface, which the kernel refuses for the whole run, and a
    // Metal1 line's substrate moved. Both costs came from the film being present in runs that
    // contain no capacitor. StackupLayer.PresentWithLayer removes that premise — the film is tied
    // to its plate and enters an EM run's medium only when the plate is one of that run's analysis
    // levels — so the module now lives on MmicGaAs() and an interconnect-only run on it extracts
    // bit-identically to the pre-module stack. Do not reintroduce a second technology; add the tie.
}
