...
public sealed partial class EmSetupEditorViewModel : ObservableObject
{
    ...
    {
        if (_suppressCommit) return;
        if (value == Working.DeembedOutsideCalibrationValidity) return;
        var before = SnapshotJson();
        Working.DeembedOutsideCalibrationValidity = value;
        CommitEdit(before, "Change de-embed outside validity");
    }
    ...
    public string? DeembedDisabledReason =>
        Working.AnalysisKind == EmAnalysisKind.CrossSection
            ? "Port de-embedding is part of the planar (full-wave) analysis. The cross-section " +
              "kernel solves a uniform line per unit length and has no calibration step to turn off."
            : null;
